...
export default function DashboardPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-app">
      ...
      {/* Topbar */}
      <header className="sticky top-0 z-40 flex items-center justify-between px-6 h-14 bg-app border-b border-line">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Primary action — always visible, no scrolling */}
          <SpawnPanel />
      ...
      {isAdmin ? (
        ...
        <div className="flex flex-col gap-4 px-6 py-4">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-5 gap-4 animate-rise" style={{ animationDelay: '120ms' }}>
            {/* Tasks — left, grows with content */}
            <div className="lg:col-span-3 bg-surface rounded-xl border border-line p-4 flex flex-col min-h-[260px] max-h-[480px]">
              <TaskTable onSelect={() => {}} selectedId={null} />
            </div>

            {/* Sessions — right, the thing admins act on most */}
            <div className="lg:col-span-2 bg-surface rounded-xl border border-line p-4 flex flex-col min-h-[260px] max-h-[480px]">
              <SessionsPanel />
            </div>
          </section>
        </div>
        ...
        /* ── User layout ─────────────────────────────────────────── */
        <section className="flex flex-1 px-6 py-10 items-start justify-center">
            ...
            <div className="bg-surface border border-line rounded-2xl p-6 flex flex-col gap-5">
              <div>
                ...
                <p className="text-sm text-fg-subtle">
                  Spawn a VS Code session with the “New session” button in the top bar.
                </p>
              </div>
              <SessionsPanel />
            </div>
