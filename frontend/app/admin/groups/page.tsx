...
export default function GroupsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 animate-rise">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl font-semibold font-display tracking-tight mb-1">Groups</h1>
        <p className="text-sm text-fg-subtle">Organise users into groups and control sandbox network egress per group.</p>
      </div>
      ...
      <div className="bg-surface border border-line rounded-xl overflow-hidden flex h-[560px]">
        {/* Groups list */}
        <div className="w-52 border-r border-line flex flex-col flex-shrink-0">
          <div className="flex-1 overflow-y-auto py-2">
            {groups.map(g => (
              <div
                key={g.id}
                onClick={() => selectGroup(g)}
                className={`flex items-center justify-between px-4 py-2.5 cursor-pointer hover:bg-raised/60 group transition-colors ${selectedGroup?.id === g.id ? 'bg-accent/10 border-r-2 border-accent' : ''}`}
              >
                <div className="min-w-0">
                  <p className={`text-sm font-medium truncate ${selectedGroup?.id === g.id ? 'text-accent' : 'text-fg-muted'}`}>{g.name}</p>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {!selectedGroup ? (
            <div className="flex-1 flex items-center justify-center text-sm text-fg-subtle">Select a group</div>
          ) : (
            <>
              {/* Tabs */}
